
#!/usr/bin/perl
# NOTE - THIS FILE GENERATED BY DBIxGenerate.pl.
# To regenerate this file, simply call the script, and it'll dump the DBIx files into the working directory.

=head1 NAME

WWW::Shopify::Model::DBIx::Schema::Result::Model::DBIx::Schema::Result::Model::AddressCustomer - Database Object for WWW::Shopify::Model::DBIx::Schema::Result::Model::AddressCustomer.

=cut

=head1 NOTICE

This is a generated class, made by WWW::Shopify/Utils/DBIxGenerate.pl. Do not modify.

=cut

use strict;
use warnings;


			package WWW::Shopify::Model::DBIx::Schema::Result::Model::AddressCustomer;
			use base qw/DBIx::Class::Core/;

			__PACKAGE__->table('shopify_addresscustomer');
			__PACKAGE__->add_columns(
				'customer_id', { data_type => 'int', is_nullable => 0 },
				'address_id', { data_type => 'int', is_nullable => 0 });
			__PACKAGE__->belongs_to(customer => 'WWW::Shopify::Model::DBIx::Schema::Result::Model::Customer', 'customer_id');
			__PACKAGE__->belongs_to(address => 'WWW::Shopify::Model::DBIx::Schema::Result::Model::Address', 'address_id');

1;